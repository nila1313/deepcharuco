...
def summarize_csv(csv_path):
    ...
    return {
        ...
        "median_error_px": sorted(errors)[len(errors) // 2] if errors else math.nan,
    }
...
for ckpt in ckpts:
    ...
    infer_img_dir = f"../my_dataset/inference_output_pseudo_sweep/{safe_name}/images"
    infer_kpt_dir = f"../my_dataset/inference_output_pseudo_sweep/{safe_name}/keypoints"
    comp_dir = f"../my_dataset/comparison_output_pseudo_sweep/{safe_name}"
...
out_path = Path("../my_dataset/pseudo_checkpoint_sweep_summary.csv")
