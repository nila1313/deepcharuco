...
from masked_data_safe_negative import SyntheticSafeNegativeDataset, PseudoSafeNegativeDataset
...
if __name__ == "__main__":
    ...
    synthetic_train = SyntheticSafeNegativeDataset(
        config,
        config.train_labels,
        config.train_images,
        visualize=False,
    ...
    synthetic_val = SyntheticSafeNegativeDataset(
        config,
        config.val_labels,
        config.val_images,
        visualize=False,
    ...
    pseudo_all = PseudoSafeNegativeDataset(
        config,
    ...
    logger = TensorBoardLogger(
        "tb_logs",
        name="deepcharuco_safe_negative_pseudo_min4"
    )

    checkpoint_callback = ModelCheckpoint(
        dirpath="tb_logs/ckpts_deepcharuco_safe_negative_pseudo_min4/",
        save_top_k=10,
        monitor="val_loss",
        mode="min",
    )
